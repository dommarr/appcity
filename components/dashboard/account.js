...
export default function Account({ user, vendor, setVendor }) {
  ...
  async function updateVendor(ven) {
    try {
      let response = await supabase.from("users").update({ vendor: ven }).eq("id", profile.id);
      return response.data;
    } catch (err) {
      alert(err);
    }
