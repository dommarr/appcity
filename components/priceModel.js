...
const PriceModel = ({ tier, model }) => {
  ...
  return (
    <div>
      <div className="-mr-5 flex justify-center items-center cursor-pointer pb-2" style={{ transition: "all .15s ease" }} onMouseEnter={openTooltip} onMouseLeave={closeTooltip} ref={btnRef}>
      ...
      <div className={(tooltipShow ? "" : "hidden ") + "bg-gray-600 border-0 mb-3 block z-50 font-normal leading-normal text-sm max-w-xs text-left no-underline break-words"} ref={tooltipRef}>
        <div>
          <div className={"bg-gray-600 text-white opacity-75 font-semibold p-3 mb-0 border-b border-solid border-gray-200 uppercase"}>{price_model} Pricing</div>
