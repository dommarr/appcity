      ...
      <div className={`price-block-2-2 ${discount && discountMessage ? "col-span-3" : "col-span-7"} ${large ? "py-4" : "py-2"} flex flex-col justify-start items-center ${search ? "" : "h-full"}`}>
        ...
        <dl className={`price-block-year ${monthly ? "hidden" : "flex"} flex-col justify-center items-center space-y-2 ${search ? "" : "h-full flex-grow"}`}>
          <div className="flex flex-col justify-center items-center text-center">
            ...
            {model !== "per-transaction" && <dt className={`${large ? "text-lg" : "text-base"} text-gray-900 font-medium`}>{primary_price_year}</dt>}
            {!tier.price_primary_text_year && <dd className={`${large ? "text-sm" : "text-sm"} text-gray-600`}>{primary_price_year_unit}</dd>}
            {tier.price_primary_text_year && model !== "revenue-fee" && model !== "one-time" && model !== "per-transaction" && (
              <dd className={`${large ? "text-xs" : "text-xs"} text-gray-400 mt-1`}>{`(yearly billing)`}</dd>
            )}
            {!tier.price_primary_text_year && model !== "revenue-fee" && model !== "one-time" && model !== "per-transaction" && model !== "minimum deposit" && (
        ...
        <dl className={`price-block-month ${monthly ? "flex" : "hidden"} flex-col justify-center items-center space-y-2 ${search ? "" : "h-full flex-grow"}`}>
          <div className="flex flex-col justify-center items-center text-center">
            ...
            {model !== "per-transaction" && <dt className={`${large ? "text-lg" : "text-base"} text-gray-900 font-medium`}>{primary_price_month}</dt>}
            {!tier.price_primary_text_month && <dd className={`${large ? "text-sm" : "text-sm"} text-gray-600`}>{primary_price_month_unit}</dd>}
            {tier.price_primary_text_month && model !== "revenue-fee" && model !== "one-time" && model !== "per-transaction" && (
              <dd className={`${large ? "text-xs" : "text-xs"} text-gray-400 mt-1`}>{`(monthly billing)`}</dd>
            )}
            {!tier.price_primary_text_month && model !== "revenue-fee" && model !== "one-time" && model !== "per-transaction" && model !== "minimum deposit" && (
